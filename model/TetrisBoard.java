...
public class TetrisBoard implements Serializable{
    ...
    public int placementHeight(TetrisPiece piece, int x) {
        return piece.getLowestYVals()[0] + this.getColumnHeight(x);
    }
...
//        }
        ...
        for (int i = 0; i < points.length; i++) {
            TetrisPoint point = points[i];
            // is it > or >=??
            if (x+point.x >= this.width || y+point.y >= this.height) {
                //this.undo();
                return ADD_OUT_BOUNDS;
            } else if (this.tetrisGrid[x+point.x][y+point.y] == true){
                //this.undo();
                return ADD_BAD;
            } else {
                this.tetrisGrid[x + point.x][y+point.y] = true;
                this.committed = false;
                this.makeHeightAndWidthArrays();
//                this.colCounts[x+point.x] = y+point.y + 1;
//                this.rowCounts[y+point.y] = this.rowCounts[y+point.y] + 1;
                if (this.getRowWidth(y+point.y) == this.width) {
                    row_filled = true;
                }
            }
        }
        if (row_filled) {
            // should clearRows be done after or when the row gets filled?? so above??
            //this.clearRows(); this doesnt belong here????
    ...
    public int clearRows() {
        ...
        for (int row = 0; row < this.height; row++) {
            if (this.getRowWidth(row) == this.width) {
                // clear the row
                // Increase count by 1 as 1 row is cleared.
                count = count + 1;
                //move things down
                for (int row2 = row; row2 < this.height; row2++) {
                    for (int col = 0; col < this.width; col++) {
                        if (row2 + 1 == this.height) {
                            this.tetrisGrid[col][row2] = false;
                        } else {
                            this.tetrisGrid[col][row2] = this.tetrisGrid[col][row2 + 1];
                        }
